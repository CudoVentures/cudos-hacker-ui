<script lang="ts">
  ...
  async function prepare() {
    ...
    const validators = await client.omni.staking
      .Validators({ status: "" })
      .then((v) => v.validators);
    const delegations = await client.omni.staking
      .DelegatorDelegations({
        delegatorAddr: client.address!,
      })
      .then((v) => v.delegationResponses);
    const unbondings = await client.omni.staking
    ...
    for (const to of delegations) {
      for (const from of validators) {
        promises.push(
          client.omni.staking
            .Redelegations({
              delegatorAddr: client.address!,
      ...
      {#each data.unbondings as unbonding}
        {#each unbonding.entries as entry}
          <tr>
            ...
            <td>{new Date(Number(entry.completionTime?.seconds) * 1000)}</td>
          </tr>
        {/each}
      {/each}
    </table>
