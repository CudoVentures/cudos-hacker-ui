...
export class Imports {
  ...
  sections(...strings: string[]) {
    const buffers = _.map(strings, s => Buffer.from(s))
    const buffer = Buffer.alloc(_.sumBy(buffers, b => b.byteLength) + 4 * strings.length)
    let i = 0
  ...
  scan(start: string, end: string, order: number) {
    ...
    for (const [key, value] of _.entries(this.contract.state)) {
      if (key >= start && key < end) {
        result.push([key, value])
      }
    }
    if (order == 1)
      return _.reverse(result.sort())
    else
      return result.sort()
  }
  ...
  db_write(key_ptr: number, value_ptr: number) {
    ...
    console.log(`db_write(${j(key)}, ${j(value)})`)
  ...
  db_scan(start_ptr: number, end_ptr: number, order: number): number {
    ...
    console.log(`db_scan(${j(start)}, ${j(end)}, ${j(order)})`)
